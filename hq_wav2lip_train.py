...
import Wav2Lip.audios as audios
...
from Wav2Lip.hparamss import hparams, get_image_list
...
class Dataset(object):
    ...
    def get_audio(self, fname):
        if not hparams.dataset_cache:
            wav = audios.load_wav(fname, hparams.sample_rate)
            return audios.melspectrogram(wav).T
        if fname in self.audio_cache:
            orig_mel = self.audio_cache[fname].copy()
        else:
            wav = audios.load_wav(fname, hparams.sample_rate)
            orig_mel = audios.melspectrogram(wav).T
